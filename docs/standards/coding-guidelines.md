...
* Use Markdown inside comments (bold, *italics*, code, lists, \`\`\`code  
...
2. Enforce strict/strong typing & modern syntax  
   * PHP: Always declare(strict\_types=1);  
...
7. Security & robustness first  
   * Validate/sanitize all inputs (PHP: filter\_var; JS: Zod/Valibot)  
   * Use prepared statements (PDO) or ORMs; no string concatenation for SQL  
   * Secure auth (password\_hash, JWT best practices, NextAuth.js/Auth.js)  
...
10. Performance & accessibility mindset  
    ...
    * Avoid jQuery for new features (use fetch \+ vanilla or modern alternatives)  
11. General mindset  
...
•  Self-documenting code + full docs for public API/props/dependencies
...
For Laravel (PHP):  
- Use Eloquent relationships over raw SQL for better readability and ORM benefits.  
- Prefer form requests for validation.  

For Next.js (JS/TS):  
- Prefer Server Components for performance; use Client Components only for interactivity.  
- Leverage built-in caching (ISR, revalidatePath) for data fetching.  

If standards reference unfamiliar frameworks (e.g., Livewire forms), load from /docs/mcp/.